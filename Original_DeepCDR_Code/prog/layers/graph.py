...
class GraphLayer(keras.layers.Layer):
    ...
    def _get_walked_edges(self, edges, step_num):
        ...
        :return: The sort_by_pred graph that has the same shape with `edges`.
        """
        if step_num <= 1:
            return edges
...
class GraphConv(GraphLayer):
    ...
    def __init__(self,
        ...
        :param units: Number of sort_by_pred states. If the input shape is (batch_size, node_num, feature_len), then the output
